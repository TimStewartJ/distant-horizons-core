...
{
    mat4 uDhProjectionInverse;
    mat4 uDhModelViewInverse;
    mat4 uDhInvMvmProj;

    mat4 uDhPrevProjMvm;

    float uCameraOffsetX;
    float uCameraOffsetY;
    float uCameraOffsetZ;
 ...
 */
vec4 calcNdc(float fragmentDepth, mat4 invMvmProj)
{
...
vec2 reprojectDh(float fragmentDepth, mat4 invMvmProj)
{
    vec4 ndc = calcNdc(fragmentDepth, invMvmProj);
...
vec4 TemporalAntiAlias(inout vec3 color, float prevAlpha)
{
    ...
    vec2 prvCoord = reprojectDh(dhDepth, uDhInvMvmProj);
