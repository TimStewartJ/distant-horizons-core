...
public class WorldGenerationQueue implements IFullDataSourceRetrievalQueue, IDebugRenderable
{
	...
	public CompletableFuture<DataSourceRetrievalResult> submitRetrievalTask(long pos, byte requiredDataDetail)
	{
		...
		synchronized (this.generationTasksByPos)
		{
			...
			{
				if (!canonicalTask.future.isDone())
				{
					return canonicalTask.future;
				}
				
				// callers' completion callbacks run before removeTerminalTask, so a finished task
				// can still be mapped here; its result must not be handed out a second time
				this.removeTerminalTask(canonicalTask);
			}

			this.generationTasksByPos.put(pos, genTask);
			this.waitingTaskByPos.put(pos, genTask);
	...
	private boolean tryStartNextWorldGenTask(DhBlockPos2D targetPos)
	{
		...
		TaskSelection<DataSourceRetrievalTask> selectedTask = selectPreferredTask(
			this.waitingTaskByPos.values(),
			this::getRunnableGenerationAvailability,
			// generate low detail LODs first
			task -> DhSectionPos.getCenterBlockPos(task.pos).chebyshevDist(targetPos) / DhSectionPos.getDetailLevel(task.pos),
		...
		synchronized (this.generationTasksByPos)
		{
			...
			if (!this.waitingTaskByPos.remove(closestTask.pos, closestTask))
			{
				return true;
			}
	...
	private byte getRawGenerationAvailability(DataSourceRetrievalTask task)
	{
		...
		byte availability = this.generator.getGenerationAvailability(
			chunkPosMin.getX(), chunkPosMin.getZ(), getAvailabilityRequestWidthInChunks(task), task.requestDetailLevel);
	...
	/**
	 * The chunk width generators received with each request before DH 3.2.1 widened
	 * {@link DataSourceRetrievalTask#widthInChunks} to the whole section:
	 * {@code 2^(sectionDetail - requestDetail - 4)}. Availability implementations derive the
	 * generated detail level from this width and the requested data detail.
	 */
	static int getAvailabilityRequestWidthInChunks(DataSourceRetrievalTask task)
	{
		int widthDetail = DhSectionPos.getDetailLevel(task.pos) - task.requestDetailLevel - 4;
		return 1 << Math.max(widthDetail, 0);
	}
	...
	private void removeTerminalTask(DataSourceRetrievalTask task)
	{
		...
		synchronized (this.generationTasksByPos)
		{
			this.waitingTaskByPos.remove(task.pos, task);
	...
	private boolean tryRequeueRejectedTask(DataSourceRetrievalTask task)
	{
		synchronized (this.generationTasksByPos)
		{
			...
			this.waitingTaskByPos.put(task.pos, task);
	...
	public void addDebugMenuStringsToList(List<String> messageList)
	{
		...
		messageList.add(
			"World Gen Queue: waiting " + this.waitingTaskByPos.size()
